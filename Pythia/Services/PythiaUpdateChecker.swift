...
final class PythiaUpdateChecker {
    ...
    private let latestReleaseURL = URL(string: "https://api.github.com/repos/douxy1994/Pythia/releases/latest")!
    ...
    func check(completion: @escaping (Result<PythiaUpdateInfo, Error>) -> Void) {
        ...
        PythiaNetworkSession.dataTask(with: request) { data, response, error in
            ...
            guard !latest.isEmpty else {
                completion(.failure(TranslationError.requestFailed("更新检查失败：没有读取到版本号。")))
                return
            }
    ...
    private static func normalizedVersion(_ raw: String) -> String {
        raw
            ...
            .replacingOccurrences(of: "Pythia", with: "", options: [.caseInsensitive])
            .trimmingCharacters(in: CharacterSet(charactersIn: " vV-_"))
    }
