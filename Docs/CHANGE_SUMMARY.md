...
- Windows 1.2.3 adds a separate input-translation shortcut, single-key global shortcuts including Insert/Home, capture of already-owned keys for visible conflict feedback, and atomic rollback. Google translation now uses web RPC with a compact-endpoint fallback instead of the rate-limited legacy endpoint.
...
- Add Authenticode signing in a later release; Windows 1.2.3 is published unsigned with an explicit SmartScreen warning and SHA-256 sidecar.
