...
- Global hotkeys: five recorder fields cover show/hide, input translation, selection translation, screenshot translation, and screenshot OCR. A temporary low-level hook lets the recorder see an already-owned key; `RegisterHotKey` replacement validates formats and duplicates, reports external ownership in a persistent Settings alert, and restores the previous runtime and visible values if any registration fails.
