...
- Both custom LLM protocols share `TranslationChunkPolicy` and one serial request state machine. Chunk boundaries operate on Swift `Character`, protect numeric tokens, retain an exact source reconstruction, and restore each chunk's whitespace envelope after translation. Each attempt owns a cancellable URLSession task plus cancellable retry work item; HTTP classification and Retry-After timing are centralized in `TranslationRetryPolicy`.
