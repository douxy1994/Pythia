...
- `GlobalHotkeys`: native `RegisterHotKey` support is implemented for show-window, selection-translate, screenshot-translate, and screenshot-OCR actions. Settings use a real keyboard recorder, normalize to the native accelerator grammar, reject malformed or duplicate combinations before saving, and restore the previous runtime set after a replacement failure; live Windows conflict validation is still required.
