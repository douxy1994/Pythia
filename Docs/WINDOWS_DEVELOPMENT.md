...
- `GlobalHotkeys`: native `RegisterHotKey` support covers show-window, selection-translate, screenshot-translate, and screenshot-OCR actions. Settings record either a single key (including Insert/Home, navigation/editing keys, F1–F24, letters, digits, and numeric-keypad keys) or a modifier combination, normalize it to the native accelerator grammar, reject malformed or duplicate shortcuts, and restore the previous runtime and visible values after a replacement failure. System ownership conflicts appear in a persistent Settings alert.
...
- `WindowBehavior`: close-to-tray, always-on-top, launch-at-startup, hide-on-blur, and window placement persistence are implemented in the native runner. Restored placement and size are clamped to the nearest connected monitor, minimum tracking size is DPI-aware, and uninstall removes the current-user Run value. Hide-on-blur uses `WM_ACTIVATE` and excludes the tray-menu interaction window.
- `WindowsAppearance`: the native host exposes system accent color and client-area animation policy, applies dark title-bar state, and requests rounded Windows 11 Mica; unsupported systems keep an opaque Fluent-style fallback. Flutter uses Segoe UI Variable with Segoe UI fallback and preserves readable opaque content surfaces.
...
- `update_checker.dart` and the settings dialog query the recent GitHub releases list, ignore legacy Pot releases that do not identify themselves as Pythia, compare the newest valid Pythia release with version `1.0.0`, and display the release URL or a clear error.
...
- `history_sync_test.dart` covers merge behavior and corrupt remote protection. `translation_service_test.dart` covers provider construction/routing and verifies that upstream error bodies are not exposed. `platform_services_test.dart` covers platform-channel method names, OCR error-code preservation, and Windows appearance contracts. `settings_dialog_test.dart` covers the settings shell at 100%, 125%, 150%, and 200% DPI. `release_package_verifier_test.dart` covers clean package, plugin payload, legacy source tree, secret marker, and missing-directory cases.
...
4. Exercise the implemented Google, Baidu, Youdao, OpenAI-compatible, DeepL, and LibreTranslate providers with real credentials and record provider-specific responses without exposing secrets.
