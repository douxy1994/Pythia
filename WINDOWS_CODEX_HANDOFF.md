...
4. 中英文混合 + 自动检测：统计中文字符数与英文单词数（连续 ASCII 字母算一个词），按主导语言决定方向——中文占主导目标为 `en`，英文占主导目标为 `zh-CN`，两者数量相等才沿用用户当前目标语言。
5. 混合文本判定目标为英文：将源侧提示为中文，目标保持英文。
6. 混合文本判定目标为中文：将源侧提示为英文，目标保持中文。

不要让单个服务再次把已解析的目标语言改回输入语言。

> ⚠️ 平台差异待同步：macOS 1.0.2 已把 `Core/PythiaCore/Sources/PythiaCore/AutomaticLanguagePolicy.swift` 从"出现一个中文字符加一个英文字母就回退到用户目标语言"改为上述主导语言计数规则（修复"中文为主夹杂少量英文术语/缩写被误判成英文"的问题）。Windows 端 `lib/core/translation_service.dart` 的 `_languageSignals` 仍是旧的存在性布尔判断，需要按同一规则重写并更新 `translation_service_test.dart` 的混合路由用例，否则两端行为分裂。
...
- 中英文混合按主导语言路由：中文字符数多于英文单词数译成英文，反之译成中文，相等时以目标语言为准。
- 标题显示 `原文（语言）` 和 `译文（语言）`。
...
- [ ] 纯中文自动到英文。
- [ ] 纯英文自动到中文。
- [ ] 混合文字按主导语言路由（中文字符数 vs 英文单词数，相等时按目标语言）。
- [ ] 多服务部分失败。
- [ ] 长文本。
- [ ] 多行、URL、代码、emoji。
