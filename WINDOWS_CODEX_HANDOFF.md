...
- 真实录制单键或组合键，包括 Insert、Home、导航/编辑键、功能键、字母数字和数字小键盘按键。
- 五个独立入口：显示/隐藏、输入翻译、划词翻译、截图翻译、截图 OCR。
- 单键和组合键统一通过 `RegisterHotKey` 注册；被其他程序占用时在设置页显示明确提示。
- 保存时先校验全部组合，再替换注册，失败时回滚运行时和输入框中的原快捷键。
