...
| 七牛 GLM 4.5 Air（free） | [`.pythia`](Plugins/qiniu-glm-4.5-air-free-1.1.1.pythia) | 七牛 API Key |
...
活跃开发基于 [github.com/douxy1994/Pythia](https://github.com/douxy1994/Pythia) 的 `master` 分支（远端名 `douxy`）。历史上的 `codex/*` 分支均为存档实验，禁止作为开发基底。其中 `codex/windows-x64-ci` 是已废弃的旧基底，缺少当前的 macOS 插件系统和输入修复；已废弃分支在树内带有 `DEPRECATED.md` 提示。指向旧上游历史的本地 `master` 分支同样为存档。
